"""
DealRoom Gradio App for HuggingFace Spaces

This is the entry point for the HuggingFace Space. It creates a standalone
Gradio interface that initializes the DealRoom environment directly.
"""
...
def _coerce_observation(data: Dict[str, Any]) -> Any:
...
def _generate_chips(view_state: Dict[str, Any]) -> List[str]:
    ...
    role_type = _role_by_id.get(selected, "")
...
def _policy_action(observation: Dict[str, Any]) -> Any:
...
def _record_step(view_state: Dict[str, Any], action: Any, obs: Any,
...
def _build_how_it_works() -> str:
    return (
        ...
        "Goal (25%) · Trust (20%) · Info (20%) · Risk (20%) · Causal (15%)</p></div>"
    )
...
def _render_all_outputs(view_state: Dict[str, Any], saved_runs: List[Dict[str, Any]]) -> Tuple[Any, ...]:
    ...
    stakeholder_choices = [
        f"{_role_icons.get(_role_by_id.get(sid, ''), '👤')} {sid}"
        for sid in stakeholders
    ]
    selected_choice = None
    if selected and selected in stakeholders:
        selected_choice = f"{_role_icons.get(_role_by_id.get(selected, ''), '👤')} {selected}"
...
def create_dealroom_gradio_app(pool: DealRoomSessionPool) -> gr.Blocks:
    ...
    with demo:
        ...
        def handle_send(msg: str, vs: Dict[str, Any], sr: List[Dict[str, Any]]) -> Tuple[Any, ...]:
            ...
            obs, reward, done, info, state = web_mgr.step_session(vs["session_id"], action)
